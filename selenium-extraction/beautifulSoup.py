    ...
    def run(self, html, checkin, checkout):
        ...
        # Escreva os dados
            ...
            for itens in resultado:
                for item in itens:
                    file.write(f"{item['Hotel']},{item['Cidade']},{item['Preço']},{item['Checkin']},{item['Checkout']},{item['Dist_centro']},{item['Dist_praia']}\n")
    ...
    # Encontre os elementos desejados e extraia informações
        for hotel in soup.find_all("div", class_="c82435a4b8 a178069f51 a6ae3c2b40 a18aeea94d d794b7a0f7 f53e278e95 c6710787a4"):
            try:
                ...
                if distance_centro:
                    distance_centro = distance_centro.text.replace(',', '.').replace(' km do centro', '')
                distance_praia = hotel.find("span", class_="abf093bdfe b058f54b9a")
                if distance_praia:
                    distance_praia = distance_praia.text.replace(' m da praia', '').replace(',', '.')
